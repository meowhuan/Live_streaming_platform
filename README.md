...
- `ADMIN_LOGIN_WINDOW_SEC`, `ADMIN_LOGIN_IP_MAX`, `ADMIN_LOGIN_USER_MAX`（管理端登录风控）
- `VIEWER_TOKEN_DAYS`
- `TURNSTILE_SECRET`
- `PUBLIC_BASE_URL`（用于通知里拼接直播地址 / 生成公开 URL）
- `TELEGRAM_BOT_TOKEN`
- `CHAT_FILTER_WORDS`
- `EMAIL_CODE_TTL_MIN`, `EMAIL_ECHO`
- `LIVE_COUNT_FALLBACK_SECS`（直播开始时间缺失时的排行榜回退范围，秒）
...
- `ADMIN_LOGIN_WINDOW_SEC`, `ADMIN_LOGIN_IP_MAX`, `ADMIN_LOGIN_USER_MAX`
- `VIEWER_TOKEN_DAYS`
- `TURNSTILE_SECRET`
- `EMAIL_CODE_TTL_MIN`, `EMAIL_ECHO`
